...
def text_to_state(file="NLP\\all_train_text.txt"):  # 将原始的语料库转换为 对应的状态文件

    if os.path.exists("NLP\\all_train_state.txt"):  # 如果存在该文件, 就直接退出
        return
    all_data = open(file, "r", encoding="utf-8").read().split("\n")  # 打开文件并按行切分到  all_data 中 , all_data  是一个list
    with open("NLP\\all_train_state.txt", "w", encoding="utf-8") as f:  # 代开写入的文件
...
class HMM:
    def __init__(self, file_text="NLP\\all_train_text.txt", file_state="NLP\\all_train_state.txt"):
    ...
    # 训练开始, 其实就是3个矩阵的求解过程
    def train(self):
        if os.path.exists("NLP\\three_matrix.pkl"):  # 如果已经存在参数了 就不训练了
            self.init_matrix, self.transfer_matrix, self.emit_matrix = pickle.load(open("NLP\\three_matrix.pkl", "rb"))
            return
        ...
        pickle.dump([self.init_matrix, self.transfer_matrix, self.emit_matrix], open("NLP\\three_matrix.pkl", "wb"))  # 保存参数
